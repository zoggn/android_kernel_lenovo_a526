...
static imgsensor_info_struct imgsensor_info = { 
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gb,//SENSOR_OUTPUT_FORMAT_RAW_B,
	.mclk = 24,
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if(gain < 64)  // gain╣дregвН╢Сж╣йг255
	{
		gain = 64;
	}
...
// tg ----------------------------------------------------------------
write_cmos_sensor(0x0000, 0x0100);		// image_orient / null
...
static void video_setting(void)
{
	...
	write_cmos_sensor(0x0B16, 0x4A8B); 
	write_cmos_sensor(0x004C, 0x0100); 
	write_cmos_sensor(0x0032, 0x0101); 
	write_cmos_sensor(0x001E, 0x0101); 
	...
	write_cmos_sensor(0x0012, 0x00A4); //x_addr_start_hact_h. 
	write_cmos_sensor(0x0018, 0x0AD3); //x_addr_end_hact_h.   
	
	write_cmos_sensor(0x0026, 0x0010); //y_addr_start_vact_h. 
	write_cmos_sensor(0x002C, 0x07B7); //y_addr_end_vact_h.   
	...
	write_cmos_sensor(0x0006, 0x07e1); //07c0); //frame_length_h 2017
	write_cmos_sensor(0x0008, 0x0B40); //line_length_h 2880
	
	write_cmos_sensor(0x000A, 0x0DB0); 
	write_cmos_sensor(0x0002, 0x04b0); //Fine.int 
	write_cmos_sensor(0x0700, 0xA0A8); //Scaler
	write_cmos_sensor(0x0A04, 0x0133);  // from 0132 to 0133
